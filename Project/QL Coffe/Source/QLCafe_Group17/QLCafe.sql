...
AS
BEGIN
SELECT b.Name AS N'Tên Bàn',tk.NGAY AS N'Ngày', tk.TongTien AS N'Tổng Tiền' FROM dbo.ThongKe tk,dbo.Ban b WHERE tk.NGAY>@Start AND tk.NGAY<@End AND tk.IDBAN = b.ID
END

GO

CREATE PROC usp_GetBaoCaoII
@Start DATETIME,
@End DATETIME
AS
BEGIN
...
CREATE PROC usp_GetBaoCaoIV
@Start DATETIME,
@End DATETIME
AS
BEGIN
SELECT b.Name AS N'Tên Bàn',tk.NGAY AS N'Ngày', tk.TongTien AS N'Tổng Tiền' FROM dbo.ThongKe tk,dbo.Ban b WHERE (tk.NGAY>@Start AND tk.NGAY<@End OR tk.NGAY = @Start) AND tk.IDBAN = b.ID
END
...
--EXEC dbo.usp_updateHD @STT = 0, -- int
--    @ID = 0 -- int
...
SELECT * FROM dbo.HoaDon WHERE STT = 0
...
--Ban
 ...
 END
  ...
  GO
  EXEC dbo.usp_chuyenban @idban1 = 0, -- int
      @idban2 = 0 -- int
 ...
 VALUES  ( 
           @IDsp , -- IDSP - int
           @Namesp , -- NameSP - nvarchar(50)
           @SOL , -- SOL - int
           @DOGIA , -- DONGIA - float
           GETDATE() , -- NgayNhap - datetime
           @Nguoinhap , -- NguoiNhap - nvarchar(50)
           @Nhacungcap , -- Nhacungcap - nvarchar(50)
           @Tongtien  -- Tongtien - float
         )
 END
 GO
 CREATE PROC usp_TKxuatkho
 @IDsp INT,
 @Namesp NVARCHAR(50),
 @SOL INT,
 @Nguoinhan NVARCHAR(50),
 ...
 VALUES  (  -- ID - int
           ...
           @Nguoinhan , -- Nguoinhan - nvarchar(50)
           @Nguoixuat  -- Nguoixuat - nvarchar(50)
         )
 END

 GO
 CREATE PROC usp_xuatkho
 @ID INT,
 @SOL INT
 AS
 BEGIN
 ...
 GO
 CREATE PROC usp_getTKnhap
 @start DATETIME,
 @end DATETIME
 AS
 BEGIN
 SELECT k.NameSP AS N'Tên sản phẩm' , k.SOL AS N'Số lượng' ,k.DONGIA AS N'Đơn giá',k.NgayNhap AS N'Ngày nhập',k.NguoiNhap AS N'Người nhập',k.Nhacungcap AS N'Nhà cung cấp' ,k.Tongtien AS N'Tổng tiền' FROM dbo.TKnhapkho k WHERE k.NgayNhap >=@start AND k.NgayNhap<=@end
 END
 GO
 SELECT * FROM dbo.TKxuatkho


 GO
 
 CREATE PROC usp_getTKxuat
 @start DATETIME,
 @end DATETIME
 AS
 BEGIN
 SELECT k.NameSP AS N'Tên sản phẩm',k.SOL AS N'Số lượng',k.Ngayxuat AS N'Ngày xuất',k.Nguoinhan AS N'Người nhận',k.Nguoixuat AS N'Người xuất' FROM dbo.TKxuatkho k WHERE Ngayxuat >=@start AND Ngayxuat<=@end
 END
